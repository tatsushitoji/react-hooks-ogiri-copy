...
export const useToggleSwitch = (props: Props) => {
  ...
  const baseStyle = useMemo(
    ...
    [state.checked, options.height, options.activeColor, options.inactiveColor],
  );
  const knobStyle = useMemo(
    () => ({
      ...
      top: '2px',
      left: state.checked ? `${options.width - options.height + 2}px` : '2px',
      backgroundColor: '#fff',
